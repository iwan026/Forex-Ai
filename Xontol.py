...
class ForexPredictor:
    ...
    def add_indicators(self, df):
        """Tambahkan indikator teknikal menggunakan pandas-ta"""
        # RSI
    ...
    def detect_support_resistance(self, df, window=20, threshold=0.02):
        """Deteksi level support dan resistance"""
        sr_levels = []

        # Mengidentifikasi pivot high dan pivot low
        for i in range(window, len(df) - window):
            ...
            if all(
                ...
                sr_levels.append((df.index[i], df["high"].iloc[i], "resistance"))
            ...
            if all(
                ...
                sr_levels.append((df.index[i], df["low"].iloc[i], "support"))

        # Mengkluster level yang berdekatan
        clustered_levels = {}
        for date, level, level_type in sr_levels:
            found_cluster = False
            for key_level, cluster in clustered_levels.items():
                if abs((level - key_level) / key_level) < threshold:
                    cluster.append((date, level, level_type))
                    found_cluster = True
                    break

            if not found_cluster:
                clustered_levels[level] = [(date, level, level_type)]

        # Mengambil level dengan konfirmasi terbanyak
        significant_levels = {}
        for key_level, cluster in clustered_levels.items():
            if len(cluster) >= 2:  # Minimal dua konfirmasi
                significant_levels[key_level] = cluster

        return significant_levels

    def detect_psychological_levels(self, df, digits=1):
        """Deteksi level psikologis (harga bulat)"""
        price_range = (df["low"].min(), df["high"].max())
        divisor = 10 ** (digits - 1)

        psych_levels = []
        start = int(price_range[0] // divisor) * divisor
        end = int(price_range[1] // divisor + 1) * divisor

        for level in range(start, end + divisor, divisor):
            zone_min = level - divisor * 0.01  # 1% buffer zone
            zone_max = level + divisor * 0.01

            # Check if price entered this zone
            price_in_zone = ((df["low"] <= zone_max) & (df["high"] >= zone_min)).any()
            if price_in_zone:
                # Count interactions with this level
                interactions = (
                    (df["low"] <= zone_max) & (df["high"] >= zone_min)
                ).sum()
                psych_levels.append((level, interactions))

        return sorted(
            psych_levels, key=lambda x: x[1], reverse=True
